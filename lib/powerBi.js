    ...
    listDatasets: function() {
        ...
        return new Promise(function(resolve, reject) {
            that.get('datasets', {}).then(function(data) {
                ...
                // Use either response.body or just body
                var _body = !!_.get(data, 'response.body')
                    ? data.response.body
                    : data.body;
                
                if (_body && /an error/i.test(_body.Message)) {
                    return reject(new Error('General error: from listDatasets: ' + _body.Message));
                }
                
                try {
                    datasets = JSON.parse(_body).value;
                }
                catch (error) {
                    // add extra logging
                    var _error = new Error('Error in listDatasets: ' + error + '\nHere is the body:\n' + data.body);
